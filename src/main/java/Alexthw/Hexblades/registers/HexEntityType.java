...
public class HexEntityType {
    ...
    static {
        ...
        FIRE_ELEMENTAL = addEntity("fire_elemental", 500, 800, 1.5f, 2.4f, true, FireElementalEntity::new, EntityClassification.MONSTER);
    ...
    static <T extends Entity> RegistryObject<EntityType<T>> addEntity(String name, int color1, int color2, float width, float height, boolean fire, EntityType.IFactory<T> factory, EntityClassification kind) {
        EntityType<T> type;
        if (fire) {
            type = EntityType.Builder.create(factory, kind)
                    .setTrackingRange(64)
                    .setUpdateInterval(1)
                    .size(width, height)
                    .immuneToFire()
