...
public class Lightning_SSwordL1 extends HexSwordItem {
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
        ...
        if (handIn == Hand.OFF_HAND && hasTwin(playerIn) && (stack.getDamage() < stack.getMaxDamage() / 2)) {
    ...
    public void onPlayerStoppedUsing(ItemStack stack, World world, LivingEntity entity, int timeleft) {
        ...
        if (!world.isRemote() && i >= 10) {
            Vector3d pos = entity.getPositionVec().add(entity.getLookVec().scale(0.5D)).add(-0.5D * Math.sin(Math.toRadians((double) (225.0F - entity.rotationYawHead))), (double) (entity.getHeight() * 0.75F), -0.5D * Math.cos(Math.toRadians((double) (225.0F - entity.rotationYawHead))));
            ...
            //world.addEntity((new SoulfireProjectileEntity((EntityType) Registry.SOULFIRE_PROJECTILE.get(), world)).shoot(pos.x, pos.y, pos.z, vel.x, vel.y, vel.z, entity.getUniqueID()));
            world.playSound((PlayerEntity) null, pos.x, pos.y, pos.z, (SoundEvent) Registry.CAST_SOULFIRE_EVENT.get(), SoundCategory.NEUTRAL, 0.75F, random.nextFloat() * 0.2F + 0.9F);

            stack.setDamage(Math.min(stack.getDamage() + 5000, stack.getMaxDamage() - 1));
        }
