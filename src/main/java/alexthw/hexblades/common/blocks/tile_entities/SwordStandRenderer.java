    ...
    public void render(SwordStandTileEntity tileEntityIn, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn, int combinedOverlayIn) {
        ...
        if (!istack.isEmpty() && mc.level != null) {
            ...
            if ((item instanceof EarthHammer1) || (item instanceof Hammer_dull)) {
                ...
                matrixStackIn.mulPose(Vector3f.ZP.rotationDegrees(45.0F));
            }
            matrixStackIn.mulPose(Vector3f.YP.rotationDegrees(2.0F * ((float) (mc.level.getGameTime() % 360L) + partialTicks)));
            ir.renderStatic(tileEntityIn.stack, ItemCameraTransforms.TransformType.GROUND, combinedLightIn, combinedOverlayIn, matrixStackIn, bufferIn);
